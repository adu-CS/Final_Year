...
def evaluate():
    ...
    set_seed(42)
    print(f"[INFO] Evaluating Vanilla MobileNetV3 on CRF 40 dataset using: {DEVICE}")

    # 1. Path to your NEWly extracted CRF 40 data
    crf40_real = r"C:\Users\srikar\ai_engineering_hub\Deepfake_detection_project\crf40_evaluation_data\real"
    crf40_fake = r"C:\Users\srikar\ai_engineering_hub\Deepfake_detection_project\crf40_evaluation_data\fake"
    
    dataset = DeepfakeDataset(crf40_real, crf40_fake, train=False)
    loader = DataLoader(dataset, batch_size=16, shuffle=False, num_workers=2, pin_memory=True)

    # 2. Load the Vanilla Model
    ...
    print("\n" + "=" * 54)
    print("      IEEE ROBUSTNESS REPORT — VANILLA MODEL (CRF 40)")
    print("=" * 54)
    ...
    # 7. Save Plots
    os.makedirs("results/crf40", exist_ok=True)
    ...
    plt.title(f'Vanilla (CRF 40) — MCC: {mcc:.4f}')
    plt.tight_layout()
    plt.savefig("results/crf40/vanilla_confusion_matrix.png", dpi=150)
    
    # ROC Curve
    fpr, tpr, _ = roc_curve(y_true, y_prob)
    plt.figure(figsize=(6, 5))
    plt.plot(fpr, tpr, color='forestgreen', label=f'Vanilla AUC={auc:.4f}')
    plt.plot([0,1],[0,1], color='gray', linestyle='--')
    plt.title('ROC Curve — Vanilla (CRF 40 Robustness)')
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/crf40/vanilla_roc_curve.png", dpi=150)
    plt.close('all')

    print("\n[SUCCESS] CRF 40 results for Vanilla saved to results/crf40/")
