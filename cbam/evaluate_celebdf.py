...
def evaluate():
    ...
    set_seed(42)
    print(f"[INFO] Evaluating Vanilla MobileNetV3 on Celeb-DF dataset using: {DEVICE}")
    ...
    print("\n" + "=" * 54)
    print("      IEEE GENERALIZATION REPORT — VANILLA MODEL (CELEB-DF)")
    print("=" * 54)
    ...
    plt.title(f'Vanilla (Celeb-DF) — MCC: {mcc:.4f}')
    plt.tight_layout()
    plt.savefig("results/celebdf/vanilla_confusion_matrix.png", dpi=150)
    ...
    plt.title('ROC Curve — Vanilla (Celeb-DF Generalization)')
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/celebdf/vanilla_roc_curve.png", dpi=150)
    plt.close('all')

    print("\n[SUCCESS] Celeb-DF results for Vanilla saved to results/celebdf/")
