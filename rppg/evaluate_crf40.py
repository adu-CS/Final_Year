...
def evaluate():
    ...
    set_seed(42)
    print(f"[INFO] Evaluating Physio (rPPG) on CRF 40 dataset using: {DEVICE}")
    ...
    print("\n" + "=" * 54)
    print("      IEEE ROBUSTNESS REPORT — PHYSIO MODEL (CRF 40)")
    print("=" * 54)
    ...
    plt.title(f'Physio (CRF 40) — MCC: {mcc:.4f}')
    plt.tight_layout()
    plt.savefig("results/crf40/physio_confusion_matrix.png", dpi=150)
    ...
    plt.title('ROC Curve — Physio (CRF 40 Robustness)')
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/crf40/physio_roc_curve.png", dpi=150)
    plt.close('all')

    print("\n[SUCCESS] CRF 40 results for Physio saved to results/crf40/")
